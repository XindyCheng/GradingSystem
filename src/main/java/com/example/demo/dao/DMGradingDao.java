...
public interface DMGradingDao {
    @Select("select * from DM_grading")
    public List<DMGrading> selectAll();

    @Select("select * from DM_grading where id=#{id}")
    public DMGrading selectById(int id);

    @Insert("<script>" + "insert into `DM_grading`" + "(time,DM_name,unit,constructability,technique,res_tl,compl_tl,"
